...
class LRMVQ(Module):
    def __init__(
        ...
        depth = 2, # number of linear layers in the codebook transform
    ):
        super().__init__()
        ...
        if not exists(codebook_transform):
            codebook_transform = nn.Sequential([nn.Linear(frozen_codebook_dim, frozen_codebook_dim) for _ in range(depth)] + [nn.Linear(frozen_codebook_dim, codebook_size)])
